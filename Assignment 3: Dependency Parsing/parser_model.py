...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = torch.nn.Parameter(torch.zeros(self.n_features * self.embed_size, self.hidden_size))
        
        torch.nn.init.xavier_uniform_(self.embed_to_hidden_weight.data)
        
        self.embed_to_hidden_bias = torch.nn.Parameter(torch.zeros(self.hidden_size))
        
        torch.nn.init.uniform_(self.embed_to_hidden_bias.data)
        
        self.dropout = nn.Dropout(p=self.dropout_prob)
        
        self.hidden_to_logits_weight = torch.nn.Parameter(torch.zeros(self.hidden_size, self.n_classes))
        
        torch.nn.init.xavier_uniform_(self.hidden_to_logits_weight.data)
        
        self.hidden_to_logits_bias = torch.nn.Parameter(torch.zeros(self.n_classes))
        
        torch.nn.init.uniform_(self.hidden_to_logits_bias.data)
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, 0, w.view(batch_size*self.n_features, )).view(batch_size, self.n_features*self.embed_size)
        ### END YOUR CODE
